...
- **Does a BAA (or some other written agreement) need to exist for this tool, and between whom?** (raised 2026-07-26, unresolved) Law firms generally aren't HIPAA covered entities, and a plaintiff's own firm receiving its client's records under the client's own authorization/subpoena typically isn't acting as a "business associate" of the provider being sued — but if the firm also does defense-side work (receiving PHI from a covered-entity client to perform services for them), that's a more classic BA relationship, and this tool would be a subcontractor in that chain. Separately, the firm's malpractice carrier may have its own vendor/data-handling requirements regardless of the strict HIPAA analysis. This is exactly a Phase 0 question — needs the firm's actual supervising attorney/compliance function (and possibly their insurance carrier) to weigh in, not a self-resolved answer.
- **Is this patentable, and should that be pursued?** (raised 2026-07-26) Assessed as unlikely to be worth pursuing: AI-assisted medical chronology extraction with citation-grounding for legal review is already a crowded commercial category (EvenUp, Filevine's "MedChron," Wisedocs, CaseFleet, Dodonai, SuperInsight, Supio, DigitalOwl, Legalyze, ProPlaintiff, Tavrn, Chronicle, and others — surfaced during an unrelated product-naming search this same week), several with the same citation-to-source-page architecture as the obvious/necessary design for this problem, which cuts against both non-obviousness and the odds of finding an underexplored, narrowly-novel technical mechanism worth claiming. Software patent-eligibility post-*Alice Corp v. CLS Bank* (2014) is also a real hurdle for anything that reads as "apply well-known AI techniques to a known problem" without a specific unconventional technical contribution. **Time-sensitive complication**: this repository is now public on GitHub — a public disclosure. In the US this starts a 1-year grace period to file (35 U.S.C. § 102(b)); in most other countries (including Europe) there is no grace period at all, and a public disclosure before filing can destroy patentability outright. Anyone who wants to preserve real optionality here should talk to a patent attorney promptly, not "eventually" — the clock may already be running. Worth also considering the flip side: whether any of the existing well-funded competitors above already hold patents this tool's own techniques could be read against, though enforcement risk for a single-firm internal tool never sold commercially is a very different (much lower) practical exposure than for a competing commercial product.
